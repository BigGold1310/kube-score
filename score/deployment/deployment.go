...
func Register(allChecks *checks.Checks, all ks.AllTypes) {
	...
	allChecks.RegisterDeploymentCheck("Deployment Strategy", `Makes sure that Deployment has correct replica ammount`, deploymentReplicas(all.Services(), all.HorizontalPodAutoscalers()))
}
	...
	return func(deployment v1.Deployment) (score scorecard.TestScore, err error) {
		...
		if !referencedByService || hasHPA {
			...
			if deployment.Spec.Replicas != nil && *deployment.Spec.Replicas >= int32(2) {
				...
				score.Grade = scorecard.GradeWarning
				score.AddComment("", "Deployment replica set", "The deployment is used by a service, not managed by HorizontalPodAutoscaler and has less then 2 replicas")
			}
		}
